...
public void addPerson(Person pers){
 ...
 try{
    PreparedStatement p= conn.prepareStatement("Insert Into person set person_id=?, "
...
public void updatePerson(Person pers){
    int oldKey = pers.getID();
      
    try{
    PreparedStatement p= conn.prepareStatement("Update person set person_id=?, "
...
public void deletePerson(Person pers){
 ...
 try{
    PreparedStatement p= conn.prepareStatement("Delete from person Where person_id=?");
    p.setInt(1, pers.getID());
    p.execute();  //use execute if no results expected back
